...
class DistributedWorker(NN_Trainer):
        ...
        for num_epoch in range(self.max_epochs):
            for batch_idx, (train_image_batch, train_label_batch) in enumerate(train_loader):
                ...
                while True:
                    ...
                    msgs,_msg_counter, encode_dur = self._encode_total_grad()
                    ...
                    print('Step time: {}, Worker: {}, Step: {}, Epoch: {} [{}/{} ({:.0f}%)], Loss: {:.4f}, Time Cost: {:.4f}, Comp: {:.4f}, Encode: {: .4f}, Comm: {: .4f}, Msg(MB): {: .4f}, Prec@1: {: .4f}, Prec@5: {: .4f}'.format(
                        time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time())), self.rank, self.cur_step, num_epoch, (batch_idx+1) * self.batch_size, len(train_loader.dataset), 
    ...
    def test_local_error(self, origin_value, code):
        ...
        keysInGroup = code['coded_index'].unpack()
        print(np.sum(keysInGroup, axis=0))
        lsstable_size = code['lsstable_size']
        decode_value = torch.Tensor(self._coder.decode(lss_table, keysInGroup, grad_number, lsstable_size))
        # code_error = (origin_value.flat[:] - decode_value.numpy().flat[:])/(np.abs(origin_value.flat[:]))
        code_error = (origin_value.flat[:] - decode_value.numpy().flat[:])
        print('code error {}'.format(code_error[:100]))
        ...
        big_score_error = np.abs((origin_value.flat[:])[big_mask] - (decode_value.numpy().flat[:])[big_mask])
        ...
        small_score_error = np.abs((origin_value.flat[:])[small_mask] - (decode_value.numpy().flat[:])[small_mask])
    ...
    def _encode_total_grad(self):
        ...
        if self.cur_step == 1:
            self.memory = torch.FloatTensor(np.zeros_like(total_grad)).cuda()
        total_grad = self.memory + torch.FloatTensor(total_grad).cuda()
        encode_start = time.time()
        ...
        if self.cur_step%10 == 1:
            if self.rank==1:
                ...
                pickle_cluster_model = pickle.dumps(self.cluster_model, protocol=2)
                self.comm.send(pickle_cluster_model, dest=0, tag=20)
                # print('compressor size{}'.format(getsizeof(pickle_cluster_model)))
            else:
                # cluster_model = None
                # req = self.comm.irecv(source=0, tag=30)
                # compressor = req.wait()
                compressor = self.comm.recv(source=0, tag=30)
        ...
        if self._coder.scheme == 'lss':
            grad_number = coded['flatten_size']
            lss_table = coded['coded_data']
            keysInGroup = coded['coded_index']
            lsstable_size = coded['lsstable_size']
            decode_grad = self._coder.decode(lss_table, keysInGroup, grad_number, lsstable_size)
            self.memory = total_grad - torch.FloatTensor(decode_grad).cuda()

        encode_dur = time.time() - encode_start
        # self.test_local_error(total_grad, coded)
        pickled = pickle.dumps(coded, protocol=2)
        # print('worker {} encode {}'.format(self.rank, p_index))
        # print("pickle size {}".format(getsizeof(pickle)))
        # byte_code = bytearray(pickled)
        byte_code = pickled
        
        _msg_counter = __count_msg_sizes(byte_code)
        
        return byte_code, _msg_counter, encode_dur
